...
NightmareWIDTH, NightmareHEIGHT = 640, 480

module NightmareTiles
  Grass = 0
  Earth = 1
end
...
def setup_game_map(filename)
  ...
  game_map.tiles = Array.new(game_map.width) do |x|
    Array.new(game_map.height) do |y|
      case lines[y][x, 1]
      when '"'
        NightmareTiles::Grass
      when '#'
        NightmareTiles::Earth
      when 'x'
...
class CptnRubyNightmare <  Gosu::Window
  def initialize
    super NightmareWIDTH, NightmareHEIGHT

    self.caption = "Cptn. Ruby"

    @sky = Gosu::Image.new("media/space.png", :tileable => true)
    @game_map = setup_game_map("media/turtle_map_nightmare.txt")
  ...
  def update
    ...
    @camera_x = [[@cptn.x - NightmareWIDTH / 2, 0].max, @game_map.width * 50 - NightmareWIDTH].min
    @camera_y = [[@cptn.y - NightmareHEIGHT / 2, 0].max, @game_map.height * 50 - NightmareHEIGHT].min
  end
