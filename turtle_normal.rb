...
NormalWIDTH, NormalHEIGHT = 640, 480
...
class CptnRubyNormal <  Gosu::Window
  def initialize
    super NormalWIDTH, NormalHEIGHT

    self.caption = "Cptn. Ruby"

    @sky = Gosu::Image.new("media/space.png", :tileable => true)
    @game_map = setup_game_map("media/turtle_map_normal.txt")
  ...
  def update
    ...
    @camera_x = [[@cptn.x - NormalWIDTH / 2, 0].max, @game_map.width * 50 - NormalWIDTH].min
    @camera_y = [[@cptn.y - NormalHEIGHT / 2, 0].max, @game_map.height * 50 - NormalHEIGHT].min
  end
