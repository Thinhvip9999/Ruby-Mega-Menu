...
class CptnRubyEasy <  Gosu::Window
  def initialize
    super EasyWIDTH, EasyHEIGHT

    self.caption = "Cptn. Ruby"

    @sky = Gosu::Image.new("media/space.png", :tileable => true)
    @game_map = setup_game_map("media/turtle_map_easy.txt")
  ...
  def update
    ...
    @camera_x = [[@cptn.x - EasyWIDTH / 2, 0].max, @game_map.width * 50 - EasyWIDTH].min
    @camera_y = [[@cptn.y - EasyHEIGHT / 2, 0].max, @game_map.height * 50 - EasyHEIGHT].min
  end
