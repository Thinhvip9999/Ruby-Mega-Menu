...
class CptnRubyHard <  Gosu::Window
  def initialize
    super HardWIDTH, HardHEIGHT

    self.caption = "Cptn. Ruby"

    @sky = Gosu::Image.new("media/space.png", :tileable => true)
    @game_map = setup_game_map("media/turtle_map_hard.txt")
  ...
  def update
    ...
    @camera_x = [[@cptn.x - HardWIDTH / 2, 0].max, @game_map.width * 50 - HardWIDTH].min
    @camera_y = [[@cptn.y - HardHEIGHT / 2, 0].max, @game_map.height * 50 - HardHEIGHT].min
  end
